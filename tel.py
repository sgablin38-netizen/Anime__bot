...
REQUIRED_CHANNELS = [
    {"name": "کانال انیمه", "username": "@Purgatory_anime_Chanel", "link": "https://t.me/Purgatory_anime_Chanel"},
    {"name": "کانال هنتای", "username": "@Hentai_of_Purgatory",    "link": "https://t.me/Hentai_of_Purgatory"},
]
# ===================================================
...
def main_reply_keyboard():
    keyboard = [
        [KeyboardButton("🎌 مشاهده انیمه‌ها")],
...
async def handle_menu_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🎌 انیمه‌ها":
        db = load_db()
        if not db:
            await update.message.reply_text(" هیچ انیمه‌ای اضافه نشده.")
            return
        keyboard = []
...
async def gm_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.from_user.id != target_uid:
        await query.answer("حق انتخواب نداری نوبت تو نیست گم شو!", show_alert=True)
        return
    ...
    if user_choice == bot_number:
        ...
        result_text = (
            f"🎲 عدد تو: *{user_choice}*\n"
            f"🤖 عدد ربات: *{bot_number}*\n\n"
            f"✅ متفاوتن! *{user_name}* از بن در رفتی ! 😅"
        )
        await query.edit_message_text(result_text, parse_mode="Markdown")
...
def main():
    ...
    app.add_handler(MessageHandler(
        filters.TEXT & ~filters.COMMAND & filters.Regex(
            "^(🎌 مشاهده انیمه‌ها|👤 ارتباط با ادمین‌ها|📢 کانال‌های ما)$"
        ),
        handle_menu_buttons
    ))
